...
export function ConversationLog({
  ...
  return (
    <div
      ref={scrollRef}
      className='flex flex-col overflow-y-auto max-h-[400px] gap-1 py-2 scroll-smooth'
    >
      {entries.map(entry => (
        <div
          key={entry.id}
        ...
        >
          ...
          <div
            className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
              entry.type === 'user' ? 'bg-purple-900/40' : 'bg-cyan-900/40'
            }`}
          >
            {entry.type === 'user' ? (
          ...
          >
            ...
            <div
              className={`flex items-center gap-2 mt-1.5 ${entry.type === 'user' ? 'justify-end' : ''}`}
            >
              {entry.action_type && (
                <span className='text-[10px] font-mono text-cyan-600 uppercase'>
                  {entry.action_type}
                </span>
              )}

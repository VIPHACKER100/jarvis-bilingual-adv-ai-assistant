    ...
    <div className='flex flex-wrap items-center gap-3 md:gap-5 p-3 glass-panel rounded-lg text-xs md:text-sm font-mono'>
      ...
      >
        ...
        <span className='font-semibold'>
          {formatPercent(bat)}
          {charging && (
            <span className='text-neon-success text-[10px] ml-0.5'>⚡</span>
          )}
        </span>
      </div>
      ...
      {/* Uptime */}
      <div
        className='flex items-center gap-1.5 text-cyan-400/70'
        title={`Uptime: ${formatUptime(uptime)}`}
        aria-label={`System uptime ${formatUptime(uptime)}`}
      >
      ...
      {status.network && (
        <div
          className='flex items-center gap-1.5 text-cyan-400/70 ml-auto'
          title={`${status.network.hostname} — ${status.network.ip}`}
          aria-label={`Network: ${status.network.ip}`}
        >

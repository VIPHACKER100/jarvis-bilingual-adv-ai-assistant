        ...
        <table className='w-full'>
          <thead>
            <tr className='border-b border-cyan-900/30'>
              <SortHeader label='PID' sortKey='pid' onSort={handleSort} />
              <SortHeader label='Name' sortKey='name' onSort={handleSort} />
              <SortHeader
                label='CPU%'
                sortKey='cpu_percent'
                onSort={handleSort}
              ...
              <SortHeader
                ...
                onSort={handleSort}
              />
              <SortHeader label='Status' sortKey='status' onSort={handleSort} />
              <SortHeader
                label='Threat'
                sortKey='threat_level'
                onSort={handleSort}
              />
          ...
          <tbody className='divide-y divide-cyan-900/20'>
            {sorted.map(proc => (
              ...
              >
                ...
                <td
                  className='px-3 py-2 text-xs font-mono text-slate-200 truncate max-w-[150px]'
                  title={proc.name}
                >
                  {proc.name}
                </td>
                  ...
                  <div className='flex items-center justify-end gap-1'>
                    <button
                      onClick={() =>
                        onAction(
                          proc.pid,
                          isSuspended(proc.status) ? 'resume' : 'suspend'
                        )
                      }

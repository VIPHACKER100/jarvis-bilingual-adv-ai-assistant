...
export function SettingsPage() {
  ...
  const [activePersonality, setActivePersonality] = useState<string | null>(
    null
  );
  const [apiKeyStatus, setApiKeyStatus] = useState<ApiKeyStatusResponse | null>(
    null
  );
        ...
        if (settingsRes.settings) {
          setForm({
            ...
            enable_dangerous_commands:
              settingsRes.settings.enable_dangerous_commands ?? false,
            confirmation_timeout:
              settingsRes.settings.confirmation_timeout ?? 30,
  ...
  const handleSaveSettings = useCallback(async () => {
    ...
    try {
      const res = await settingsApi.update({
        llm_provider: form.llm_provider as
          | 'openrouter'
          | 'nvidia'
          | 'openai'
          | 'google'
    ...
    async (id: string) => {
      ...
      try {
        ...
        addNotification({
          ...
          message:
            err instanceof Error ? err.message : 'Failed to set personality',
          type: 'error',
          duration: 5000,
        });
      } finally {
    ...
    <div className='min-h-screen bg-cyber-dark p-4 md:p-6'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-6xl mx-auto'>
        ...
        <Card className='p-5'>
          ...
          <div className='space-y-4'>
            ...
            <SettingsToggle
              ...
              onChange={checked =>
                setForm(f => ({ ...f, wake_word_enabled: checked }))
              }
            />
            ...
            {form.wake_word_enabled && (
              <Input
                ...
                onChange={e =>
                  setForm(f => ({ ...f, wake_word_phrase: e.target.value }))
                }
                maxLength={50}
            ...
            <SettingsToggle
              ...
              onChange={checked =>
                setForm(f => ({ ...f, enable_dangerous_commands: checked }))
              }
            />
            ...
            <div className='flex flex-col gap-1.5'>
              ...
              <input
                ...
                onChange={e =>
                  setForm(f => ({
                    ...f,
                    confirmation_timeout: parseInt(e.target.value) || 30,
                  }))
                }
                className='w-full px-4 py-2.5 bg-cyber-surface/60 backdrop-blur-md border border-cyan-800/30 rounded-lg font-mono text-sm text-slate-200 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/50'
          ...
          <div className='space-y-4'>
            {apiKeyFields.map(({ key, label, providerKey }) => (
              <ApiKeyCard
                ...
                onChange={val =>
                  setKeyInputs(prev => ({ ...prev, [key]: val }))
                }
                onTest={() =>
                  handleTestKey(key, keyInputs[key as keyof typeof keyInputs])
                }
                onSave={() =>
                  handleSaveKey(key, keyInputs[key as keyof typeof keyInputs])
                }
              />
            ))}
          </div>
        ...
        <Card className='p-5'>
          ...
          {personalityLoading && (
            <LoadingOverlay visible={true} message='Changing personality...' />
          )}
            ...
            {personalities.map(p => (
              <PersonalityCard
                ...
                isActive={
                  ...
                  (activePersonality === null && p.id === 'stark')
                }

...
export function AnalyticsPage() {
  ...
  const fetchCommands = useCallback(async () => {
    try {
      ...
      addNotification({
        id: crypto.randomUUID(),
        title: 'Error',
        message: 'Failed to load command insights',
        type: 'error',
        duration: 5000,
      });
    }
  ...
  const handleProcessAction = useCallback(
    async (pid: number, action: 'suspend' | 'resume' | 'terminate') => {
      ...
      try {
        ...
        addNotification({
          ...
          title: 'Action Failed',
          message:
            err instanceof Error ? err.message : `Failed to ${action} process`,
          type: 'error',
          duration: 5000,
        });
      } finally {
      ...
      <div className='max-w-6xl mx-auto'>
        ...
        {activeTab === 'commands' && (
          <div className='space-y-6'>
            ...
            {/* Top Commands */}
            <Card className='p-4'>
              <h2 className='font-display font-bold text-sm text-slate-200 mb-3'>
                Top Commands
              </h2>
            ...
            {/* Daily Activity */}
            {commandInsights?.daily_activity &&
              commandInsights.daily_activity.length > 0 && (
                <PerformanceChart
                  data={commandInsights.daily_activity.map(d => ({
                    timestamp: d.date,
                    value: d.count,
                  }))}
            ...
            {/* Failure Patterns */}
            {commandInsights?.failure_patterns &&
              commandInsights.failure_patterns.length > 0 && (
                <Card className='p-4'>
                  <h2 className='font-display font-bold text-sm text-slate-200 mb-3'>
                    Failure Patterns
                  </h2>
                  ...
                  <table className='w-full'>
                    <thead>
                      <tr className='border-b border-cyan-900/30'>
                        <th className='px-3 py-2 text-left text-[10px] font-mono font-semibold uppercase text-slate-400'>
                          PID
                        </th>
                        <th className='px-3 py-2 text-left text-[10px] font-mono font-semibold uppercase text-slate-400'>
                          Process

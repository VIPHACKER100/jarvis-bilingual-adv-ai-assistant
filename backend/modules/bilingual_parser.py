...
class BilingualParser:
    ...
    def detect_language(self, text: str) -> str:
        ...
        hindi_words = {'kholo', 'band', 'karo', 'chalao', 'bhejo', 'kaun', 'kya', 'hai',
                      'samay', 'tareekh', 'din', 'aaj', 'kal', 'suno', 'sun', 'raha',
                      'mujhe', 'tum', 'aap', 'namaste', 'shukriya', 'dhanyavad', 'kaise',
                      'madad', 'sakte', 'ho', 'btao', 'batao', 'dekhna', 'ruko', 'dheere',
                      'tez', 'badhao', 'kam', 'aawaz', 'awaz', 'par', 'ko', 'me', 'se',
                      'ka', 'ki', 'aur', 'kahan', 'kab', 'kyu', 'mausam', 'tapman',
                      'garmi', 'sardi', 'hisab', 'jodo', 'ghatao', 'guna', 'bhag'}
    ...
    def parse_command(self, text: str) -> Tuple[str, str, Optional[str]]:
        ...
        if any(word in text_lower for word in ['shutdown', 'turn off', 'power off']):
    ...
    def get_response(self, response_key: str, lang: str, *args) -> str:
        ...
        template = responses.get(response_key, RESPONSES['en'].get(response_key, 'Unknown response'))

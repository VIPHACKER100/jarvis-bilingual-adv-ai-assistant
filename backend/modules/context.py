...
class ContextManager:
    ...
    INTENT_PATTERNS = {
        ...
        "farewell": [r"\b(bye|goodbye|see you|take care|alvida|phir milenge)\b", r"^(stop|exit|quit|band karo)"],
        "gratitude": [r"\b(thank|thanks|shukriya|dhanyavad|thank you)\b"],
        "question": [r"\b(what|who|where|when|why|how|kya|kaun|kahan|kab|kyu|kaise)\b", r"\?"],
        "urgent": [r"\b(urgent|quick|fast|immediately|jaldi|turant)\b"],
        "frustrated": [r"\b(not working|error|problem|issue|stupid|damn|hell|not again)\b"],
        "follow_up": [r"\b(and|also|too|plus|aur|bhi|phir)\b"],
    }
    ...
    async def update_context(self, user_input: str, command_type: str, success: bool, session_id: str = "") -> None:
        ...
        logger.info(
            f"Context updated: topic={self.current_context.active_topic}, mood={self.current_context.user_mood}"
        )
    ...
    async def update_mobile_context(self, device_id: str, data: Dict[str, Any]) -> None:
        ...
        if battery.get("level") and battery.get("level") < 15 and not battery.get("is_charging"):
            ...
            self.set_context_variable(
                "urgent_mobile_alert",
                f"Your phone '{data.get('device_name', 'Mobile')}' is at {battery['level']}% battery. You should plug it in soon, Sir.",
            )
    ...
    def get_alternative_suggestion(self, command_key: str, params: Any, language: str = "en") -> Optional[str]:
        ...
        if command_key == "open_app":
            ...
            for key, url in web_mappings.items():
                if key in app_name:
                    return (
                        f"Sir, I couldn't find {key} on your system. Would you like me to open the web version at {url} instead?"
                        if language == "en"
                        else f"सर, मुझे आपके सिस्टम पर {key} नहीं मिला। क्या आप चाहेंगे कि मैं इसके बजाय {url} पर वेब वर्शन खोलूँ?"
                    )

            return (
                f"Sir, I couldn't find '{params}'. Should I search for it online?"
                if language == "en"
                else f"सर, मुझे '{params}' नहीं मिला। क्या मुझे इसे ऑनलाइन खोजना चाहिए?"
            )

        if command_key == "file_management":
            return (
                "I couldn't find the file. Should I search in a different directory or check the Recycle Bin?"
                if language == "en"
                else "मुझे फ़ाइल नहीं मिली। क्या मुझे किसी दूसरी डायरेक्टरी में खोजना चाहिए?"
            )
    ...
    async def suggest_next_action(self) -> Optional[str]:
        ...
        if not self.current_context.last_successful and self.current_context.last_command_type:
            alt = self.get_alternative_suggestion(
                self.current_context.last_command_type, self.current_context.last_command
            )
            if alt:
                return alt
                ...
                if "my favorite" in text_lower:
                    ...
                    if pref_match:
                        ...
                        await memory_manager.save_memory(
                            MemoryEntry(key=key, value=val, category="preferences", source="conversation")
                        )
                        continue

                await memory_manager.save_memory(
                    MemoryEntry(key=base_key, value=value, category=category, source="conversation", confidence=0.8)
                )
    ...
    def export_context(self) -> Dict:
        ...
        return {
            ...
            "intent_history": [
                {"primary_intent": h.primary_intent, "confidence": h.confidence, "entities": h.entities}

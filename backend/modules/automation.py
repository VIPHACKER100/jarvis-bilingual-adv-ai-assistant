...
class ScheduledTask:
    ...
    days: List[str] = None  # For weekly: ['monday', 'wednesday', etc.]
    enabled: bool = True
    created_at: str = ""
    last_run: str = ""
    next_run: str = ""
...
class Macro:
    ...
    name: str
    description: str
    commands: List[Dict]  # List of {command: str, delay: int, parameters: dict}
...
class AutomationManager:
    ...
    def _save_data(self):
        ...
        try:
            ...
            with open(macros_file, 'w') as f:
                json.dump([asdict(macro) for macro in self.macros.values()], f, indent=2)
    ...
    def create_task(self, name: str, description: str, command: str,
                   ...
                   days: List[str] = None, parameters: Dict = None) -> Optional[ScheduledTask]:
    ...
    def toggle_task(self, task_id: str) -> bool:
        ...
        self._save_data()
        logger.info(f"{'Enabled' if task.enabled else 'Disabled'} task: {task.name}")
        return True
    
    # ==================== MACROS ====================
    
    def create_macro(self, name: str, description: str, commands: List[Dict],
                    trigger: str, trigger_phrase: str = "", hotkey: str = "") -> Optional[Macro]:
        """Create a new macro"""
        try:
            import uuid
    ...
    def toggle_macro(self, macro_id: str) -> bool:
        ...
        self._save_data()
        logger.info(f"{'Enabled' if macro.enabled else 'Disabled'} macro: {macro.name}")
        return True
        ...
        presets = [
            {
                ...
                'commands': [
                    {'command': 'open_app', 'delay': 2, 'parameters': {'app': 'chrome'}},
                    {'command': 'open_app', 'delay': 2, 'parameters': {'app': 'vscode'}},
                    {'command': 'open_app', 'delay': 2, 'parameters': {'app': 'spotify'}}
                ],
                'trigger': 'voice',
            ...
            },
            {
                ...
                'commands': [
                    {'command': 'close_app', 'delay': 1, 'parameters': {'app': 'vscode'}},
                    {'command': 'show_desktop', 'delay': 0, 'parameters': {}}
                ],
                'trigger': 'voice',

...
class WhatsAppManager:
    ...
    async def open_whatsapp_web(self, language: str = "en") -> Dict:
        ...
        try:
            ...
            return {"success": True, "action_type": "WHATSAPP_WEB", "method": "web", "response": response}

        except Exception as e:
            logger.error(f"Error opening WhatsApp Web: {e}")
            return {"success": False, "action_type": "WHATSAPP_WEB", "error": str(e)}
    ...
    async def open_whatsapp_desktop(self, language: str = "en") -> Dict:
        ...
        try:
            ...
            return {"success": True, "action_type": "WHATSAPP_DESKTOP", "method": "desktop", "response": response}
    ...
    async def send_message_web(self, contact: str, message: str, language: str = "en") -> Dict:
        ...
        try:
            ...
            return {"success": False, "action_type": "WHATSAPP_MESSAGE", "error": str(e)}

    async def send_message_desktop(
        self, contact: str, message: str, language: str = "en", confirmed: bool = False
    ) -> Dict:
        ...
        if not confirmed:
            return {
                ...
                "contact": contact,
                "message": message,
                "response": f"Send message to {contact}: '{message[:30]}...'?"
                if len(message) > 30
                else f"Send message to {contact}: '{message}'?",
                "confirmation_context": {"command": "whatsapp_send_desktop", "contact": contact, "message": message},
            }
        ...
        try:
            ...
            log_command(f"WhatsApp message to {contact}", "whatsapp_message", True)
    ...
    async def call_contact(self, contact: str, video: bool = False, language: str = "en") -> Dict:
        ...
        try:
            ...
            return {"success": False, "action_type": "WHATSAPP_CALL", "error": str(e)}
    ...
    async def get_known_contacts(self, language: str = "en") -> Dict:
        ...
        for alias, info in self.contacts_map.items():
            if isinstance(info, dict):
                contacts.append({"alias": alias, "name": info.get("name", ""), "phone": info.get("phone", "")})
            else:
                contacts.append({"alias": alias, "name": info, "phone": ""})
    ...
    async def get_status(self, language: str = "en") -> Dict:
        ...
        status_text = (
            ...
            else "WhatsApp Desktop is not installed (using Web fallback)"
        )
        if is_running:
    ...
    async def draft_smart_reply(self, language: str = "en") -> Dict:
        ...
        try:
            ...
            draft = await llm_module.get_response(text=visual_context, language=language, context=prompt)
            ...
            if draft:
                ...
                return {"success": True, "draft": draft, "response": response}

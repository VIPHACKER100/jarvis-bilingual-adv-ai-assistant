...
export const ArcReactor: React.FC<ArcReactorProps> = ({ isActive, onClick, language }) => {
  ...
  return (
    <div className="relative flex items-center justify-center p-10 cursor-pointer group" onClick={handleClick}>
      ...
      <div className={`absolute w-80 h-80 rounded-full border border-slate-700/50 border-dashed ${isActive ? 'animate-spin-slow' : 'opacity-30'}`}></div>

      {/* --- Middle Reactor Ring (Fast Rotate) --- */}
      <div className={`absolute w-72 h-72 rounded-full border-2 border-cyan-900/40 ${isActive ? 'animate-spin-reverse-slow' : ''}`}>
        <div className="absolute top-0 left-1/2 w-2 h-4 bg-cyan-500/50 transform -translate-x-1/2"></div>
        <div className="absolute bottom-0 left-1/2 w-2 h-4 bg-cyan-500/50 transform -translate-x-1/2"></div>
        <div className="absolute left-0 top-1/2 h-2 w-4 bg-cyan-500/50 transform -translate-y-1/2"></div>
        <div className="absolute right-0 top-1/2 h-2 w-4 bg-cyan-500/50 transform -translate-y-1/2"></div>
      </div>

      {/* --- Inner Energy Ring (Pulsing) --- */}
      <div className={`absolute w-64 h-64 rounded-full border-4 border-slate-800 flex items-center justify-center transition-all duration-500 shadow-2xl ${isActive ? 'shadow-[0_0_50px_rgba(6,182,212,0.6)] scale-100' : 'scale-95 opacity-80'
        }`}>
      ...
      <button
        className={`
          ...
          overflow-hidden
          ${isActive
            ? 'bg-slate-900 border-4 border-cyan-400 shadow-[inset_0_0_60px_rgba(6,182,212,0.5)]'
      ...
      >
        ...
        <div className="relative z-20 flex flex-col items-center">
          <span className={`text-base font-bold tracking-[0.2em] transition-colors duration-300 ${isActive ? 'text-cyan-100' : 'text-slate-500'}`}>
            {isActive
              ? (language === 'hi' ? 'ON' : 'ONLINE')
              : (language === 'hi' ? 'OFF' : 'OFFLINE')
            }
          </span>
          <div className={`h-1 w-16 mt-1 rounded-full transition-all duration-300 ${isActive ? 'bg-cyan-400 shadow-[0_0_10px_cyan]' : 'bg-slate-700'}`}></div>
        </div>
